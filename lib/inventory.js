...
import { logger, logError, startTimer, withDbTimer } from './logger';
...
export class InventoryService {
  ...
  static async getStock(productId) {
    try {
      ...
      logError(error, { method: 'getStock', productId });
      return 0;
    }
  }
  ...
  static async updateStock(productId, quantity, operation = 'set') {
    try {
      ...
      logError(error, { method: 'updateStock', productId, quantity, operation });
      throw error;
    }
  }
  ...
  static async reserveStock(productId, quantity) {
    try {
      ...
      logError(error, { method: 'reserveStock', productId, quantity });
      throw error;
    }
  }

  /**
   * Atomically reserve stock using an optimistic lock.
   * Uses updateMany with stock >= qty condition — if stock was
   * already taken by a concurrent request, count will be 0 and
   * this throws. Safe for concurrent access.
   */
  static async reserveStockAtomic(productId, quantity) {
  ...
  static async getLowStock(threshold = 10) {
    try {
      ...
      logError(error, { method: 'getLowStock', threshold });
      return [];
    }
  }
  ...
  static async getOutOfStock() {
    try {
      ...
      logError(error, { method: 'getOutOfStock' });
      return [];
    }
  }
  ...
  static async getInventorySummary() {
    try {
      ...
      logError(error, { method: 'getInventorySummary' });
      return {
        totalProducts: 0,
        totalStock: 0,
        lowStockCount: 0,
  ...
  static async bulkUpdateStock(updates) {
    try {
      ...
      logError(error, { method: 'bulkUpdateStock', count: updates?.length });
      throw error;
    }
  }
}
