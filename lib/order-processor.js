...
export async function createOrder(data) {
  ...
  timer.stopAndLog(`Order created: ${order.orderNumber}`, "info", {
    orderId: order.id,
...
export async function processOrderQueue(batchSize = 5) {
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    try {
      ...
      logError(err, { context: 'processOrderQueue', orderId });
      ...
      try {
        ...
        logError(releaseErr, { context: 'releaseStock', orderId });
      }
  ...
  timer.stopAndLog(`Processed ${processed.length} orders`, "info", {
    batchSize,
    processed: processed.length,
    confirmed: processed.filter((p) => p.status === 'CONFIRMED').length,
